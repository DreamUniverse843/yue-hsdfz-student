...
def getSpecificScore(rootURL,isTotal):
    ...
    if isTotal == 0:
        ...
        try:
            ...
            print("==========基本情况==========")
            print(paperResult.iloc[0])
            groupGradePatt = re.compile(r"var grade =(.*?);$", re.MULTILINE | re.DOTALL)  # 学年得分率特征
            groupSubjectPatt = re.compile(r"var avsname =(.*?);$", re.MULTILINE | re.DOTALL) # 学年学科特征
            groupGradeAttr = groupGradePatt.search(testSpecificResult.find("script",text=groupGradePatt).text).group(1) # 学年数据
            GroupSubjectAttr = groupSubjectPatt.search(testSpecificResult.find("script",text=groupSubjectPatt).text).group(1) #
            print("\n\n=========学年情况=========")
            print("学年得分率: " + groupGradeAttr)
            print("对应科目: " + GroupSubjectAttr)
        except (ValueError):
            print(Fore.RED + "\n没有多科数据，可能未合成总成绩表。\n")
